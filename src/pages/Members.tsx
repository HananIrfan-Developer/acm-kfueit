...
export function Members() {
  ...
  // Sort members: Presidents/Vice Presidents first, then others
  const sortedMembers = [...members].sort((a, b) => {
    const roleA = a.role.toLowerCase();
    const roleB = b.role.toLowerCase();
    if (roleA.includes('president') && !roleB.includes('president')) return -1;
    if (!roleA.includes('president') && roleB.includes('president')) return 1;
    return 0;
  });
              ...
              >
                ...
                <div className="flex justify-center gap-4 mt-auto relative z-10">
                  {member.socialLinks?.linkedin && (
                    <a href={member.socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all border border-slate-100">
                      <Linkedin size={18} />
                    </a>
                  )}
                  {member.socialLinks?.github && (
                    <a href={member.socialLinks.github} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-all border border-slate-100">
                      <Github size={18} />
                    </a>
                  )}
                  {member.socialLinks?.twitter && (
                    <a href={member.socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:text-blue-400 hover:bg-blue-50 transition-all border border-slate-100">
                      <Twitter size={18} />
                    </a>
                  )}
                </div>
